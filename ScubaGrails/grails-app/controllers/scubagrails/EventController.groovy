...
class EventController {
    ...
    def show = {
        ...
        if (!eventInstance) {
            flash.message = "${message(code: 'event.not.found.message', args: [message(code: 'event.label', default: 'Event'), params.id])}"
            redirect(action: "index")
        }
        else {
            def model = [eventInstance: eventInstance, occurrenceStart: eventInstance.getStartTime(), occurrenceEnd: eventInstance.getEndTime()]
    ...
    def save = {
		...
		// Calendrier francais : 
		try {
			Date startTime = params.date('startTime', ['dd/MM/yyyy hh:mm a'])
			Date endTime = params.date('endTime', ['dd/MM/yyyy hh:mm a'])
			eventInstance.setStartTime(startTime);
			eventInstance.setEndTime(endTime);
			
			if (eventInstance.save(flush: true)) {
				flash.message = "Evénement crée avec succès"
			...
			flash.message = "Erreur lors de la création de l'événement"
			render(view: "create", model: [eventInstance: eventInstance])
		}
    ...
    def update = {
        ...
        if (!result.error) {
            flash.message = "${message(code: 'event.updated.message', args: [message(code: 'event.label', default: 'Event'), eventInstance.id])}"
            redirect(action: "index")
        }
