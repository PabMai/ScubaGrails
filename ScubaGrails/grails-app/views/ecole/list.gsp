	...
	<body>
		...
		<g:if test="${session?.user?.admin}">
			...
			<ul>
				...
				<li><g:link class="create" action="create"><g:message code="ecole.new.label" args="[entityName]" /></g:link></li>
			</ul>
		</div>
		<div id="list-ecole" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="nom" title="${message(code: 'ecole.nom.label', default: 'Nom')}" />
				...
				<g:each in="${ecoleInstanceList}" status="i" var="ecoleInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${ecoleInstance.id}">${fieldValue(bean: ecoleInstance, field: "nom")}</g:link></td>
						<td>${ecoleInstance.abonnes?.count?.size}</td>
