...
function setupRecurSavePopups() {
    $(".delete.recurring").live('click', function() {
        $("#deletePopup").dialog({
            title: "Supprimer événement récurrent",
            width: 500,
            modal: true
         });
    ...
    var editPopup = $("#editPopup").dialog({
        title: "Editer événement récurrent",
        width: 500,
        modal: true,
        autoOpen: false
    });
...
function getRecurDescription() {
    ...
    if ($("#isRecurring").is(":checked")) {
        ...
        if (recurInterval == 1) {
            ...
            description += "Chaque " + recurInterval + " " + getRecurTypeUnit(recurType);
        }
        ...
        if (recurCount) {
            description += ", " + recurCount + " fois" ;
        }
        else if (recurUntil) {
            description += ", jusqu\'au " + recurUntil;
        }
